...
namespace Restaurant1.Controllers
{
    public class AdminController : Controller
    {
        ...
        public PartialViewResult AFoodPartials(string str)
        {
            if (str == null)
            {
                ...
                else
                {
                    ...
                    var l = (AdminFilter)(Session["filter"]);
                    ...
                    AdminFilter f = (AdminFilter)Session["filter"];
        ...
        public JsonResult Situation(Situation s)
        {
            ...
            using (var db = new RestaurantEntities12())
            {
                ...
                foreach (var item in list)
                {
                    ...
                    {
                        int a = db.Database.ExecuteSqlCommand("Update AdminOrder set OrderSitutation =" + 5 + "where UserID=" + item.UserID + "and OrderDate='" + item.OrderDate + "'");
                        var upInvent = (from i in db.AdminOrder where i.OrderSitutation == 5 && i.OrderDate == d && i.UserID == sit select i).ToList();
        ...
        public JsonResult Filter(AdminFilter f)
        {
            Session["filter"] = f;
        ...
        {
            RestaurantEntities12 db = new RestaurantEntities12();
            List<CoruierInfo> courirerList = new List<CoruierInfo>();
            var list = db.Kurye.ToList();
            foreach (var item in list)
            {
                CoruierInfo c = new CoruierInfo();
                c.CoruierPhone = "+9";
                c.CoruierNumber =(int) item.KuryeNumber;
                c.ID = item.KuryeID;
        ...
        public JsonResult AddCoruier(CoruierInfo cr)
        {
            RestaurantEntities12 db = new RestaurantEntities12();
            var control = (from a in db.Kurye where a.KuryeNumber == cr.CoruierNumber select a).FirstOrDefault();
            if(control ==null)
            {
                SHA1 sha = new SHA1CryptoServiceProvider();
                string psw = cr.CoruierPassword;
                string pswHash = Convert.ToBase64String(sha.ComputeHash(System.Text.Encoding.UTF8.GetBytes(psw)));
                Kurye x = new Kurye();
                x.KuryeNumber = cr.CoruierNumber;
        ...
        public JsonResult RemoveCoruier(CoruierInfo c)
        {
            RestaurantEntities12 db = new RestaurantEntities12();
